...
void main() {
    ...
    for(int i=0; i< numLights;i++){
        if (lightSources[i].lightType == 1) { 
            ambient += calculatePointLight(lightSources[i], normal, viewDir);
        } 
        else if (lightSources[i].lightType == 2) { // SPOTLIGHT
            diffuse += calculateSpotLight(lightSources[i], normal, viewDir);
        }
        else if (lightSources[i].lightType == 3) { // DIRECTIONAL LIGHT
            specular += calculateDirectionalLight(lightSources[i], normal, viewDir);
        } 
    }
